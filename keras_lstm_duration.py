...
def get_sequences():
    ...
    for song_index in range(len(musicobj.data)):
        ...
        for chord_index in range(0,len(musicobj.data[song_index])-num_timesteps,1):
            ...
            output_sequences.append(musicobj.data[song_index][chord_index+num_timesteps][0][2])
            temp_in = []
            for chord in chord_set:
                temp_in.append(chord[0][2])
            input_sequences.append(temp_in)

    num = len(input_sequences)
    input_sequences = np.reshape(input_sequences,(num,num_timesteps,1))
    input_sequences = input_sequences/float(len(musicobj.duration_dict))
...
def create_network(network_input):
    ...
    model.add(Dense(256))
    model.add(Dropout(0.3))
    model.add(Dense(len(musicobj.duration_dict)))
...
def train(model,network_input,network_output):
    filepath = "./TrainingData/KERAS-LSTM-DURATION/weights-{epoch:02d}-{loss:.4f}.hdf5"
